...
class Point {
  ...
  // Returns a particle at a random position away from the origin.
  randomPosition() {
  ...
  // Returns true if the given particle extends past an edge of the canvas.
  isOutOfBounds(p) {
    const [cx, cy] = this.center;
    const [px, py] = [p.x + cx, p.y + cy];
  ...
  // Updates the model with the new particle added to the cluster.
  update(p, running) {
    const distance = p.length();

    // Check if the new particle is now the farthest from the origin.
    ...
    // Check if the cluster has grown past an edge of the canvas.
  ...
  randomWalk({ running, probability, requestId }) {
    ...
    while (running.current) {
      ...
      if (p.isCloseTo(n)) {
        ...
        // Particle is pushed away if it fails to stick.
    ...
    // The cluster continues to grow if the DLA simulation is still running.
    if (running.current) {
